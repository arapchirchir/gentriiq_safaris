...
<body class="h-full bg-[#FAF6F0] text-[#211915] antialiased transition-colors duration-200 dark:bg-[#180D08] dark:text-[#FAF6F0]"
    ...
    <div class="min-h-full">
        ...
        <div class="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-72 lg:flex-col">
            <div class="flex grow flex-col gap-y-5 overflow-y-auto border-r border-black/10 bg-[#24140E] px-6 pb-4 text-[#FAF6F0]">
                <!-- Brand Header -->
                <div class="flex h-20 shrink-0 items-center justify-between border-b border-white/10">
                    <a href="{{ route('admin.dashboard') }}" class="group flex items-center gap-3">
                        <div class="flex size-10 items-center justify-center rounded-sm bg-[#D96B27] text-white font-black text-xl">
                            G
                        </div>
                        <div>
                            <span class="block font-serif text-sm font-black tracking-widest text-[#FAF6F0]">GENTRIIQ</span>
                            <span class="block text-[10px] font-bold uppercase tracking-wider text-[#D96B27]">Management Portal</span>
                        </div>
                    </a>
                </div>
                ...
                <div class="rounded-sm border border-white/10 bg-black/20 p-3">
                    <div class="flex items-center gap-3">
                        <div class="flex size-9 items-center justify-center rounded-full bg-[#D96B27]/20 font-bold text-sm text-[#D96B27]">
                            {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                        </div>
                        <div class="min-w-0 flex-1">
                            <p class="truncate text-xs font-bold text-white">{{ auth()->user()->name }}</p>
                            <span class="inline-block rounded-xs bg-[#D96B27]/20 px-1.5 py-0.5 text-[10px] font-bold text-[#D96B27]">
                ...
                <nav class="flex flex-1 flex-col">
                    <ul role="list" class="flex flex-1 flex-col gap-y-7">
                        <li>
                            <div class="text-[11px] font-bold uppercase tracking-wider text-white/40">Core Operations</div>
        ...
        <div class="lg:pl-72 flex flex-col min-h-screen">
            <!-- Top Navbar -->
            <header class="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-black/10 bg-white px-4 shadow-xs sm:gap-x-6 sm:px-6 lg:px-8 dark:border-white/10 dark:bg-[#24140E]">
                <button type="button" @click="mobileSidebarOpen = true" class="-m-2.5 p-2.5 text-gray-700 lg:hidden dark:text-white">
                ...
                <div class="flex flex-1 items-center justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <button type="button" @click="toggleTheme()"
                            ...
                            <svg x-show="!isDark" class="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
                            </svg>
                            <svg x-show="isDark" class="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" style="display: none;">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <main class="flex-1 py-8 px-4 sm:px-6 lg:px-8">
                ...
                @if (session('success'))
                    <div class="mb-6 rounded-sm border border-emerald-500/30 bg-emerald-500/10 p-4 text-emerald-900 dark:text-emerald-200">
                        <div class="flex items-center gap-3">
                            <svg class="size-5 shrink-0 text-emerald-600 dark:text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
