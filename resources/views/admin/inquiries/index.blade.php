<x-layouts.admin title="Safari Proposals & Inquiries">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-black text-[#211915] sm:text-2xl dark:text-white">
                    Safari Proposals & Customer Inquiries
                </h2>
            ...
            <form method="GET" action="{{ route('admin.inquiries.index') }}" class="flex items-center gap-2">
                ...
                @endif
                <div class="relative">
                    <input type="text" name="q" value="{{ $search }}" placeholder="Search guest or ref..."
                    ...
                    <tbody class="divide-y divide-black/5 dark:divide-white/5 font-medium text-[#211915] dark:text-white">
                        @forelse ($inquiries as $inquiry)
                            <tr class="hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                                ...
                                <td class="py-3.5 px-4">
                                    <div>{{ $inquiry->travel_date?->format('M d, Y') ?? $inquiry->travel_month . ' ' . $inquiry->travel_year }}</div>
                                    @if ($inquiry->travel_season)
                                        <div class="text-[10px] text-[#6E635C] dark:text-[#FAF6F0]/60">{{ $inquiry->travel_season }}</div>
                                    @endif
                                </td>
                                <td class="py-3.5 px-4">
                                    <div>{{ $inquiry->duration_label }}</div>
                                    <div class="text-[11px] text-[#6E635C] dark:text-[#FAF6F0]/60">{{ $inquiry->traveller_label }}</div>
                                </td>
                                <td class="py-3.5 px-4">
